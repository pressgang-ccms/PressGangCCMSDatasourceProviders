...
public class RESTEntityCache {
    ...
    public void add(final RESTBaseEntityV1<?, ?, ?> value, final Number id, final boolean isRevision) {
        // Add a null revision to specify it's the latest
        if (!isRevision) {
            add(value, id.toString(), null);
        }

        // Add the revision as well if it's not null so that revision can be looked up
        if (value.getRevision() != null) {
            add(value, id.toString(), value.getRevision());
        }
    }
    ...
    public <T extends RESTBaseEntityV1<T, ?, ?>> void expire(final Class<T> clazz, final Integer id) {
