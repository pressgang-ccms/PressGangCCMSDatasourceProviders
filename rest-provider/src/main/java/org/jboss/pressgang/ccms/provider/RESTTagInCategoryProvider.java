...
public class RESTTagInCategoryProvider extends RESTTagProvider implements TagInCategoryProvider {
    ...
    public CollectionWrapper<TagInCategoryWrapper> getTagInCategoryRevisions(int id, Integer revision, final RESTBaseCategoryV1<?, ?, ?> parent) {
        final Integer categoryId = parent.getId();
        final Integer categoryRevision = ((RESTBaseEntityV1ProxyHandler<RESTTagV1>) ((ProxyObject) parent).getHandler()).getEntityRevision();
        ...
        try {
            ...
            for (final RESTTagInCategoryCollectionItemV1 tagItem : category.getTags().getItems()) {
                ...
                if (tag.getId() == id && (revision == null || tag.getRevision().equals(revision))) {
                    return getWrapperFactory().createCollection(tag.getRevisions(), RESTTagInCategoryV1.class, true, parent);
                }
            }
    
        } catch (Exception e) {
            log.error("Unable to retrieve the Revisions for TagInCategory " + id + (revision == null ? "" : (", Revision " + revision)),
                    e);
        }
